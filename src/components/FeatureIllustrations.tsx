...
const TranscriptLines = () => (
  <>
    <p className="text-[7px] font-bold text-[#1F2A37]/40 uppercase tracking-widest flex-shrink-0">
      Transcript
    </p>
    <p
      className="text-[8px] text-[#1F2A37]/70 leading-relaxed"
      style={{
        animation: "ft-line1 9s ease-in-out infinite",
    ...
    <p
      ...
      style={{
        animation: "ft-line2 9s ease-in-out infinite",
        willChange: "opacity,transform",
      }}
    >
      <span
      ...
      >
        She go to office yesterday.
      </span>
    </p>
    <p
      className="text-[8px] text-[#1F2A37]/70 leading-relaxed"
      style={{
        animation: "ft-line3 9s ease-in-out infinite",
        willChange: "opacity,transform",
      }}
    >
...
const ScorePanel = ({ className = "" }: { className?: string }) => (
  <div
    className={`bg-white rounded-xl border border-[#6FA9D6]/20 p-2.5 flex flex-col gap-1.5 overflow-hidden ${className}`}
  >
    <div className="flex items-center justify-between flex-shrink-0">
      <span className="text-[6.5px] font-bold text-[#1F2A37]/40 uppercase tracking-widest">
        Fluency
      </span>
    ...
    <div className="h-1.5 rounded-full bg-[#6FA9D6]/12 overflow-hidden flex-shrink-0">
      <div
        ...
        style={{
          background: "linear-gradient(90deg,#6FA9D6,#4A6FA5)",
          animation: "ft-score 7s ease-in-out 0.5s infinite",
          willChange: "width",
        }}
      />
    </div>
    <div className="flex flex-col gap-1 flex-shrink-0">
      {(
        [
          ["Vocab", "82%", "#10b981", "ft-vocab"],
          ["Grammar", "54%", "#f59e0b", "ft-grammar"],
        ] as const
      ).map(([label, val, color, anim]) => (
        <div key={label} className="flex items-center gap-1">
          <span className="text-[5.5px] text-[#1F2A37]/40 w-7 flex-shrink-0">
            {label}
          </span>
          <div className="flex-1 h-1 rounded-full bg-[#1F2A37]/8 overflow-hidden">
            <div
              ...
              style={{
                background: color,
                animation: `${anim} 7s ease-in-out 0.3s infinite`,
                willChange: "width",
              }}
            />
          </div>
          <span
            className="text-[5.5px] font-bold flex-shrink-0"
            style={{ color }}
          >
            {val}
          </span>
...
const VideoPanel = ({ className = "" }: { className?: string }) => (
  <div
    className={`rounded-xl bg-[#e8f0f8] relative overflow-hidden flex items-center justify-center ${className}`}
  >
    <div className="absolute inset-0 flex items-end p-2 gap-1">
      {(["#6FA9D6", "#4A6FA5", "#6FA9D6"] as const).map((c, i) => (
        <div
          ...
          style={{ height: `${28 + i * 14}%`, background: c, opacity: 0.22 }}
        />
      ))}
    </div>
    ...
    <span className="absolute top-1.5 right-1.5 text-[6px] font-mono font-bold text-white bg-black/30 px-1.5 py-0.5 rounded">
      24:18
    </span>
  </div>
);
  ...
  <div className="absolute inset-0 bg-[#f4f8fc] flex flex-col md:flex-row gap-2.5 p-3 overflow-hidden">
    ...
    <div className="flex-1 min-w-0 min-h-0 bg-white rounded-xl border border-[#6FA9D6]/20 shadow-sm p-3 flex flex-col gap-2 overflow-hidden">
      <div className="flex items-center justify-between gap-1 flex-shrink-0">
        ...
        <span className="text-[7px] font-semibold px-1.5 py-0.5 rounded-full bg-violet-50 text-violet-500 border border-violet-200">
          ✦ AI
        </span>
      </div>
      <div className="flex-shrink-0 rounded-lg bg-[#f4f8fc] border border-[#6FA9D6]/15 px-2.5 py-1.5 flex items-center gap-1.5 overflow-hidden">
        <div className="flex-1 h-1.5 rounded-full bg-[#1F2A37]/10 overflow-hidden">
          <div
            ...
            style={{
              animation: "ft-type 5s ease-in-out infinite",
              willChange: "width",
            }}
          />
        </div>
        <div
          className="w-px h-3 bg-[#4A6FA5] flex-shrink-0"
          style={{
      ...
      <div className="flex flex-wrap gap-1 flex-shrink-0">
        {(["Vocabulary", "Grammar", "Advanced"] as const).map((label, i) => (
          <span
            ...
            style={{
              animation: `ft-chip${i + 1} 8s ease-in-out infinite`,
              willChange: "opacity,transform",
            }}
          >
            {label}
        ...
        {(
          ...
          <div key={i} className="flex items-center gap-1.5">
            ...
            <div
              ...
              style={{
                width: w,
                animation: `ft-shimmer 1.8s ease-in-out ${d}s infinite`,
                willChange: "opacity",
              }}
            />
          </div>
    ...
    <div className="flex-1 min-w-0 min-h-0 flex flex-col gap-1.5 overflow-hidden">
      <p className="text-[7px] font-semibold text-[#1F2A37]/40 uppercase tracking-widest flex-shrink-0">
        Saved
      </p>
      {(
        [
        ...
        >
          <div className="min-w-0 flex-1">
            <p className="text-[8px] font-semibold text-[#1F2A37] truncate">
              {title}
            </p>
          ...
          <div className="w-5 h-5 rounded-full bg-[#6FA9D6]/15 flex items-center justify-center flex-shrink-0 ml-2">
            <svg width="8" height="10" viewBox="0 0 10 12" fill="none">
              <path
                d="M2 1h6a1 1 0 011 1v9l-4-2.5L1 11V2a1 1 0 011-1z"
                stroke="#4A6FA5"
                strokeWidth="1.2"
                fill="none"
                strokeLinejoin="round"
      ...
      <div className="flex items-center gap-1.5 mt-auto flex-shrink-0">
        ...
        <span className="text-[6.5px] text-[#4A6FA5]/50 font-medium whitespace-nowrap">
          + generate more
        </span>
        <div className="flex-1 h-px bg-[#6FA9D6]/20" />
      </div>
  ...
  <div className="absolute inset-0 bg-white flex flex-col gap-2 p-3 overflow-hidden">
    <div className="flex items-center justify-between flex-shrink-0">
      <span
        ...
        style={{
          animation: "ft-live 2s ease-in-out infinite",
      ...
      >
        <span
          ...
          style={{
            animation: "ft-slot-pulse 1.2s ease-in-out infinite",
            willChange: "opacity",
          }}
        />
        Live Now
      ...
      {(
        ...
        >
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[7px] text-[#1F2A37]/40">
              {time} · {peers} {peers === 1 ? "peer" : "peers"}
            </p>
          </div>
          {live && (
    ...
    <div className="relative flex justify-center pb-5 flex-shrink-0">
      <button
        ...
        style={{
          ...
          animation: "ft-glow 2.5s ease-in-out 0.5s infinite",
          willChange: "box-shadow",
        }}
      >
        Join Session
  ...
  <div className="absolute inset-0 bg-white flex flex-col gap-2 p-3 overflow-hidden">
    <div className="flex items-center justify-between flex-shrink-0">
      ...
      <span className="flex items-center gap-1 text-[7.5px] font-bold text-rose-500 bg-rose-50 px-2 py-1 rounded-full border border-rose-200">
        <span
          className="w-1 h-1 rounded-full bg-rose-500 flex-shrink-0"
          style={{
            animation: "ft-slot-pulse 1.2s ease-in-out infinite",
            willChange: "opacity",
          }}
      ...
      <div className="w-full h-full rounded-xl border-2 border-[#1F2A37]/10 bg-[#f4f8fc] overflow-hidden flex flex-col">
        ...
        <div className="h-3 bg-[#1F2A37]/6 flex items-center px-2 gap-1 flex-shrink-0">
          {([0.18, 0.12, 0.08] as const).map((o, i) => (
            <div
              key={i}
              className="w-1.5 h-1.5 rounded-full bg-[#1F2A37]"
              style={{ opacity: o }}
            />
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 grid-rows-2 gap-1 p-1.5 flex-1 min-h-0">
          {([true, false, false, false] as const).map((isHost, i) => (
            <div
              ...
              style={{
                background: isHost
                  ? "linear-gradient(135deg,#4A6FA5,#6FA9D6)"
                  : "#e8f0f8",
                animation: isHost
            ...
            >
              <svg width="14" height="18" viewBox="0 0 28 32" fill="none">
                <ellipse
                  ...
                  rx="7"
                  ry="8"
                  fill={isHost ? "rgba(255,255,255,0.35)" : "#6FA9D6"}
                  opacity="0.7"
                />
                <ellipse
                  cx="14"
                  ...
                  rx="11"
                  ry="7"
                  fill={isHost ? "rgba(255,255,255,0.2)" : "#6FA9D6"}
                  opacity="0.4"
                />
              </svg>
              {isHost ? (
                <>
                  <span className="absolute top-1 left-1 text-[5px] font-bold text-white/90 bg-white/20 px-1 py-0.5 rounded-full">
                    HOST
                  </span>
                  <div className="absolute bottom-1 left-1/2 -translate-x-1/2 flex gap-0.5 items-end h-3">
                    {WAVE_ANIMS.map((anim, j) => (
                      <div
                        ...
                        style={{
                          height: "2px",
                          animation: `${anim} ${WAVE_DURS[j]} ease-in-out ${j * 0.08}s infinite`,
                          willChange: "height",
                        }}
                      />
                    ))}
                ...
                <span className="absolute bottom-1 text-[5px] text-[#6FA9D6]/70 font-medium">
                  ⊙ Ready
                </span>
              )}
            </div>
    ...
    <div className="flex items-center justify-between flex-shrink-0">
      ...
      <div className="relative pb-5">
        <button
          ...
          style={{
            background: "linear-gradient(135deg,#6FA9D6 0%,#4A6FA5 100%)",
            animation: "ft-glow 2.5s ease-in-out 0.8s infinite",
            willChange: "box-shadow",
          }}
        >
          Start Hosting
    ...
    <div className="md:hidden h-full flex flex-col gap-2 p-3">
      ...
      <div className="flex-shrink-0 h-1.5 rounded-full bg-[#6FA9D6]/15 overflow-hidden">
        <div
          ...
          style={{
            background: "linear-gradient(90deg,#6FA9D6,#4A6FA5)",
            animation: "ft-progress 7s ease-in-out infinite",
            willChange: "width",
          }}
        />
      </div>
      ...
      <div className="flex-shrink-0 flex items-start gap-2">
        ...
        <div className="relative flex-shrink-0 pb-5 pt-0.5">
          <button
            ...
            style={{
              background: "linear-gradient(135deg,#6FA9D6 0%,#4A6FA5 100%)",
              animation: "ft-glow 2.5s ease-in-out 1.2s infinite",
              willChange: "box-shadow",
            }}
          >
            Review
...
const features = [
  {
    title: "Generate & Save",
    description:
      "Create custom activities tailored to your goals and save them for later.",
  },
  {
    title: "Join Sessions",
...
const FeatureIllustrations = () => (
  <>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 md:grid-rows-2 gap-4 md:h-[720px]">
      ...
      <motion.div
        className="md:col-span-2 rounded-2xl p-6 md:p-8 flex flex-col gap-4 md:gap-5 group hover:shadow-lg transition-shadow overflow-hidden relative bg-white border border-gray-200"
        style={{ minHeight: 0 }}
      ...
      >
        ...
        <div className="flex-1 relative overflow-hidden rounded-xl border border-gray-200 min-h-[220px] md:min-h-0">
      ...
      <motion.div
        className="rounded-2xl p-5 md:p-6 flex flex-col gap-3 md:gap-4 group hover:shadow-lg transition-shadow overflow-hidden relative bg-white border border-gray-200"
        style={{ minHeight: 0 }}
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
